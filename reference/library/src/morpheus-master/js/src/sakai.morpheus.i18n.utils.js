(function ($) {
    ...
    portal.i18n.loadProperties = function (options) {
        ...
        if (!options.names || !options.path || !options.namespace) {
            console && console.log('You must supply names, path and namespace.');
            return;
        }

        if (options.debug) {
            console && console.log('names: ' + options.names);
            console && console.log('path: ' + options.path);
            console && console.log('namespace: ' + options.namespace);
        }
        ...
        $.i18n.properties({
            ...
            callback: function () {
                ...
                if (options.debug) {
                    console && console.log('Updated translations: ');
                    console && console.log(portal.i18n.translations[options.namespace]);
                }
    ...
    portal.i18n.translate = function (namespace, key, options) {
        ...
        if (!namespace || !key) {
            console && console.log('You must supply a namespace and a key.');
            return;
        }
        ...
        if (!ret) {
            console && console.log(namespace + '#key ' + key + ' not found. Returning key ...');
            return key;
        }
